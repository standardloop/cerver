...
static char *makeVersionLiteral(HTTPLexer *lexer)
{
    ...
    if (!found_slash)
    {
        // Log(ERROR, "didn't find / in http version");
        return NULL;
    }
    ...
    if (lexer->current_char == CARRIAGE_CHAR)
    {
        backtrackChar(lexer);
    }
...
static char *makeStringLiteral(HTTPLexer *lexer)
{
    ...
    while (ALWAYS)
    {
        ...
        if (isSpecialCharacter(lexer->current_char) || start_till_max == max_header_length)
        {
            break;
        }
    }
...
extern HTTPToken *HTTPLex(HTTPLexer *lexer)
{
    ...
    if (lexer->line == 1 && isFirstCharOfMethod(lexer->current_char))
    {
        // maybe it is HTTP?
        if (peakCharHTTPLexer(lexer) == 'T')
        {
        ...
        {
            char *method_literal = makeMethodLiteral(lexer);
            if (method_literal == NULL)
            {
                token = NewHTTPToken(HTTPTokenIllegal, curr_pos, lexer->position + 1, lexer->line, NULL);
                lexer->last_token = HTTPTokenIllegal;
            ...
            {
                token = NewHTTPToken(HTTPTokenMethod, curr_pos, lexer->position + 1, lexer->line, method_literal);
    ...
    else if (lexer->current_char == FORWARDLASH_CHAR && lexer->line == 1)
    {
    ...
    else if (lexer->line > 1)
    {
        // if we have encountered the double CRLF \r\n\r\n then we need to get the body
        if (lexer->double_crlf)
        {
